...
<small>
    ...
    < ?= htmlentities('<img src="http://acafe.org.br/site/admin/arquivos/[NOME_ARQUIVO].[EXTENCAO]" style="width: 100%" />'); ?><br>
    -->
...
<!-- IMAGEM -->
<? $campo = ['IMAGEM_INTERNA[]', 'Imagem iterna <small>"Opcional", <small>marcar Não próximo campo para aparecer</small>)</small>', 'file'] ?>
<label><?= $campo[1] ?></label>
<input type="<?= $campo[2] ?>" id="<?= $campo[0] ?>" name="<?= $campo[0] ?>" >

<!-- IMAGEM_APARECER_DETALHE -->
<? $campo = ['IMAGEM_APARECER_DETALHE', 'Imagem de chamada na notícia?', 'select', 100, ' required '] ?>
<label for="<?= $campo[0] ?>"><?= $campo[1] ?>:</label>
<select id="<?= $campo[0] ?>" name="<?= $campo[0] ?>" <?= $campo[4] ?>>
